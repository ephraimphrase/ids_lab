...
capture_benign.py
=================
Standalone script to capture ONLY benign (normal) traffic.
Useful for capturing a specific normal event (e.g. a ping, file transfer)
without running the full pipeline.

Run this script on the VICTIM VM BEFORE generating traffic from the Attacker VM.

Usage:
    # Use --interface enp0s3 for VirtualBox, or --interface ens33 for VMware.
    sudo python3 capture_benign.py --interface ens33 --extra-filter "icmp and host 10.0.0.10 and host 10.0.0.20"

    # Use an absolute outdir to avoid files being saved to /root/ when using sudo:
    sudo python3 capture_benign.py --interface ens33 --outdir /home/ubuntu/captures
"""
...
def main():
    ...
    p.add_argument(
        "--extra-filter",
        default="",
        help="BPF filter string (e.g., 'icmp and host 10.0.0.10').",
    )
    ...
    with CaptureSession(
        ...
        if args.duration > 0:
            ...
            try:
                input("  [Capture running] Generate your normal traffic now. Press ENTER to stop: ")
