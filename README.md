...
| **Hydra: "optional parameters must have the format X=value" / "no valid optional parameter type given: F"** | Hydra's `http-get-form`/`http-post-form` syntax splits its argument on unescaped colons. Two rules: (1) the condition field is a **bare string by default, meaning "failure"** — there is no `F=` prefix in hydra's grammar, only `S=` exists (to flip it to a *success* string), so adding `F=` is itself invalid and throws the second error above; (2) any literal colon inside an optional field's value — like the `Cookie: PHPSESSID=...` header — must be escaped as `\:`, or hydra misparses the field boundaries and throws the first error above. Use the corrected command in the table above (bare condition string, `H=Cookie\: ...`). |
