...
To guarantee a perfectly clean dataset with zero internet background noise, run this exact command on the Victim VM. It instructs the engine to capture the full attack pipeline while strictly filtering for packets that involve your Victim VM.
...
*(**Important Note on the Filter:** Filter by the **Victim's** IP, not the Attacker's. `DDoSSYNFlood`'s `--rand-source` spoofs the source IP on every packet, so a filter like `"host <attacker_ip>"` matches nothing during that phase and silently produces a 0-byte capture — `"host <victim_ip>"` matches every phase correctly since all attack traffic is destined to the Victim regardless of what source IP it claims. This doesn't affect labelling/verification, which work off packet contents read from the pcap afterward, not the capture-time filter.)*
...
| **DDoSSYNFlood capture is 0 bytes even though hping3 shows real traffic sent** | `--rand-source` spoofs the source IP on every packet, so it never matches the attacker's real IP. If the pipeline's `--extra-filter` is set to `"host <attacker_ip>"` (attacker-centered — the old default in this doc), every DDoS packet gets silently filtered out at capture time while every other attack phase captures fine, since only DDoS spoofs its source. Fix: use `"host <victim_ip>"` instead — victim-centered, matches every phase including DDoS, since all attack traffic is destined to the Victim regardless of claimed source. If you already lost a DDoS capture to this, redo just that one attack with `capture_ddos.py --extra-filter "dst host <victim_ip> and tcp"` rather than re-running the whole pipeline. |
